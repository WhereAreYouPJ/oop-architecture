...
public class MemberController {
    ...
    @PostMapping(value = "/email/send/v2", name = "메일 전송 v2")
    @Operation(summary = "Mail Send API v2", description = "인증 메일 전송 v2 API")
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "요청에 성공하였습니다.",
    ...
    public ResponseEntity<BaseResponse<String>> sendMailV2(@Valid @RequestBody MailSendRequest request) {
        ...
        // 이메일 유효성 검사
        memberService.validateEmail(request.email());
    ...
    @Operation(summary = "Modify profileImage API", description = "프로필 사진 변경 API",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE,
                            schema = @Schema(implementation = ModifyProfileImageRequest.class))
            )
    )
    @ApiResponses(value = {
            @ApiResponse(
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(
                    responseCode = "KN007",
                    description = "404 카카오 회원가입이 필요합니다.",
                    content = @Content(
                            schema = @Schema(
                                    implementation = GlobalExceptionHandler.ErrorResponse.class)))
    })
    public ResponseEntity<BaseResponse<LoginResponseDto>> kakaoLogin(@Valid @RequestBody SnsLoginRequest request) {

        // 검사
        request.snsLoginRequestValidate();

        // Vo to Dto
        SnsRequestDto kakaoLoginRequest = request.toSnsLoginRequest();

        // 서비스
        LoginResponseDto loginResponseDto = memberService.kakaoLogin(kakaoLoginRequest);

        return ResponseEntity.ok().body(BaseResponse.ofSuccess(HttpStatus.OK.value(),loginResponseDto));
    }

    @PostMapping(value = "/kakaoJoin", name = "카카오 회원가입")
    @Operation(summary = "join Kakao Member API", description = "카카오 회원가입 API")
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "요청에 성공하였습니다.",
            ...
            @ApiResponse(
                    ...
                    content = @Content(
                            schema = @Schema(
                                    implementation = GlobalExceptionHandler.ErrorResponse.class))),
            @ApiResponse(
                    responseCode = "KDCE008",
                    description = "409 Kakao Id 중복 오류",
                    content = @Content(
                            schema = @Schema(
                                    implementation = GlobalExceptionHandler.ErrorResponse.class)))
    })
    public ResponseEntity<BaseResponse<String>> JoinKakaoMember(@Valid @RequestBody MemberRequestVo.SnsJoinRequest request) {
        ...
        // VO -> DTO 변환
        SnsJoinRequestDto kakaoJoinRequest = request.toSnsJoinRequest();
        memberService.joinKakaoMember(kakaoJoinRequest);
